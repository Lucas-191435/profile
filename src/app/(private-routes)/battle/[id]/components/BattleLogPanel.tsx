...
export function BattleLogPanel({ battle, myParticipantId, currentUserId }: BattleLogPanelProps) {
  ...
  return (
    <>
      {/* Aba fixa presa na borda direita da viewport — nunca disputa espaço com a cena da
          batalha, então sempre visível pra abrir/fechar o histórico, mesmo com o painel aberto. */}
      <button
        onClick={() => {
          sounds.clickMouse.play();
          setOpen((o) => !o);
        }}
        className={`fixed top-4 z-50 flex h-9 w-9 items-center justify-center rounded-l-lg border-2 border-r-0 border-[#404058] bg-card text-muted-foreground transition-[right] duration-300 ease-out hover:text-foreground active:scale-90 ${
          open ? "right-80 sm:right-96" : "right-0"
        }`}
      ...
      {/* Backdrop — sobrepõe a batalha (em vez de espremê-la) e fecha o painel ao ser tocado. */}
      {open && (
        <div
      ...
      <div
        className={`fixed inset-y-0 right-0 z-50 flex h-full w-80 sm:w-96 flex-col border-l-4 border-[#404058] bg-card transition-transform duration-300 ease-out ${
